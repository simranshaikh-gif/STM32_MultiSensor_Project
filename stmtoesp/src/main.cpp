    ...
    if (this.readyState == 4) {
      if (this.status == 200) {
        ...
        try {
          ...
          document.getElementById("val").innerHTML = obj.value;
  ...
  if (Serial2.available()) {
    ...
    if (data.length() > 0) {
      ...
      if (data.indexOf("TOUCHED") != -1) {
        touchState = "1";
        currentValue = "0";
        Serial.println(">> PARSED: TOUCHED (State=1)");
      } else if (data.indexOf("RELEASED") != -1) {
        touchState = "0";
        currentValue = "0";
        Serial.println(">> PARSED: RELEASED (State=0)");
      } else {
        // Fallback
        Serial.println(">> PARSED: UNKNOWN format");
      ...
      Serial.print("RX Raw: ");
      Serial.println(data);
      Serial.print("Current TouchState Variable: ");
